"""Main ETL orchestrator — loads Billboard JSON data into SQLite."""
...
def _load_hot100(conn, directory: str, only_dates=None):
    ...
    cur = conn.cursor()
    cur.execute("SELECT id, title, artist_credit FROM songs;")
    for row in cur.fetchall():
        song_cache[(row['title'], row['artist_credit'])] = row['id']
    
    cur.execute("SELECT id, name FROM artists;")
    for row in cur.fetchall():
        artist_cache[row['name']] = row['id']
    ...
    for i, (chart_date, file_path) in enumerate(files):
        ...
        for entry in entries:
            ...
            entry_rows.append((
                ...
                entry["weeks"], 1 if entry["is_new"] else 0,
            ))
...
def _load_b200(conn, directory: str, only_dates=None):
    ...
    # Pre-load caches from DB
    cur = conn.cursor()
    cur.execute("SELECT id, title, artist_credit FROM albums;")
    for row in cur.fetchall():
        album_cache[(row['title'], row['artist_credit'])] = row['id']
